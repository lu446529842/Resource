...
def parse_filter_summary_xml(file = None):
    """
	...
	for sub_g  in g_elements :
	    G_metric = element_filter_by_string(g_elements,"METRICS")
	    G_hosts = element_filter_by_string(g_elements,"HOSTS")
	    G_cluster = element_filter_by_string(g_elements,"CLUSTER")


	    #root=>grids=>grid=>metrics
	    ganglia_data["grids"][G_NAME]["metrics"] = {}
		...
		#root=>grids=>grid=>metrics>metric
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]={}
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["name"] = M_NAME
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["sum"] = M_SUM
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["type"] = M_TYPE
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["units"] = M_UNITS
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["slope"] = M_SLOPE
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["source"] = M_SOURCE
		

		#root=>grids=>grid=>metrics>metric=>extra_data
		ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["extra_data"] = {}
		...
		#root=>grids=>grid=>metrics>metric=>extra_data=>extra_elememt
		    ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["extra_data"][E_NAME]={}
		    ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["extra_data"][E_NAME]["name" ] = E_NAME
		    ganglia_data["grids"][G_NAME]["metrics"][M_NAME]["extra_data"][E_NAME]["val" ] = E_VAL 

		#root=>grids=>grid=>hosts
	    ganglia_data["grids"][G_NAME]["hosts"] = {}
	    for hosts in G_hosts :
		H_UP = hosts.getAttribute("UP")
		H_DOWN = hosts.getAttribute("DOWN")
		H_SOURCE = hosts.getAttribute("SOURCE")
		ganglia_data["grids"][G_NAME]["hosts"]["up"] = H_UP
		ganglia_data["grids"][G_NAME]["hosts"]["down"] = H_DOWN
		ganglia_data["grids"][G_NAME]["hosts"]["source"] = H_SOURCE
...
if __name__ == "__main__":
  filter_data =  parse_filter_summary_xml("/root/mygit/ganglia_xml_data/xml_data/filter_summary")
  print_dic(filter_data)
